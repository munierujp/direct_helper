(function(){
  ...
  /** ストレージ設定キー */
  const STORAGE_SETTINGS_KEY = 'direct_helper_settings';
...
詳しい使用方法は<a href="https://github.com/munierujp/direct_helper/blob/master/README.md" target="_blank">readme</a>を参照してください。`,
  ...
  */
  function doActions(){
    fetchSettings().then(settings => {
      Object.keys(SETTINGS_KEY_ACTIONS).filter(key => settings[key] === true).map(key => SETTINGS_KEY_ACTIONS[key]).forEach(action => action());
    });
  }
  ...
  */
  function doWatchMessage(){
    fetchSettings().then(settings => {
      ...
      const updateTalkData = $talkItems => {
        //デフォルト監視対象を監視対象に追加
      ...
      $talkLists.each((i, talkList) => {
        //トークデータを更新
        const $talkItems = $(talkList).find('.talk-list-item');
        updateTalkData($talkItems);

        //トーク一覧に子ノード追加時、トークデータを更新
  ...
  */
  ...
  /**
  * Chrome SyncSync Storageから設定をフェッチします。
  * @return {Promise} 設定をフェッチするPromise
  */
  function fetchSettings(){
    return new Promise(resolve => {
      chrome.storage.sync.get(STORAGE_SETTINGS_KEY, items => resolve(items[STORAGE_SETTINGS_KEY]));
    });
  }
